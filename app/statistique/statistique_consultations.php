...
$this->Line($x6,$y,$x6,$y+$dy+$ddy);//ligne verticale en x5

$y = $y+$dy;
$this->Text($x0+$dx,$y,' N');
$this->Text($x1+25*$dx,$y,'Dates');
$this->Text($x2+$dx,$y,'Généraliste');
$this->Text($x3+$dx,$y,'Spécialiste');
$this->Text($x4+$dx,$y,'Pré-Anesthesiste');
$this->Text($x5+$dx,$y,'TOTAL');
$y=$y+$ddy;
$this->Line($x0,$y,$x6,$y);//ligne horizontale de l'en-tete de colonne
}
//Pied de page
function Footer()
{
...
while($res1 = mysql_fetch_array($req1))
{
	...
	while($res2 = mysql_fetch_array($req2))
	{
		...
		$nbre_pre_anesthesiste_patient=0;
		while($res3 = mysql_fetch_array($req3))
		{
			...
			if($ref_prest==1103){$nbre_pre_anesthesiste_patient = 1;}
		}
		$nbre_generaliste = $nbre_generaliste + $nbre_generaliste_patient;
		$nbre_specialiste = $nbre_specialiste + $nbre_specialiste_patient;
		$nbre_pre_anesthesiste = $nbre_pre_anesthesiste + $nbre_pre_anesthesiste_patient;
		$nbre_total = $nbre_generaliste + $nbre_specialiste + $nbre_pre_anesthesiste;
	}
		...
		$pdf->Line($x6,$y,$x6,$y+$dy+$ddy);//ligne verticale en x5
		...
		$pdf->setXY($x4+$dx,$y-$dy);$pdf->Cell($l5-2*$dx,$dy+2*$ddy,''.number_format($nbre_pre_anesthesiste,0,","," ").'',0,0,'C');
		$pdf->setXY($x5+$dx,$y-$dy);$pdf->Cell($l6-2*$dx,$dy+2*$ddy,''.number_format($nbre_total,0,","," ").'',0,0,'C');

		$y = $y+$ddy;
		$pdf->Line($x0,$y,$x6,$y);//ligne horizontale de l'en-tete de colonne
		if($y>$ymax)
		{
		$pdf->AddPage();
		$y=$y1;
	...
	$total_pre_anesthesiste = $total_pre_anesthesiste+$nbre_pre_anesthesiste;
	...
	$pdf->SetXY($x4+$dx,$y-$dy);$pdf->Cell($l5-2*$dx,$dy+2*$ddy,''.number_format($total_pre_anesthesiste,0,","," ").'',0,0,'C');
	$pdf->SetXY($x5+$dx,$y-$dy);$pdf->Cell($l6-2*$dx,$dy+2*$ddy,''.number_format($total_total,0,","," ").'',0,0,'C');

	$pdf->Line($x0,$y+$ddy,$x6,$y+$ddy);//ligne horizontale de fin de colonne
 	if($y>$ymax)
	{
		$pdf->AddPage();
		$y=$y1;
