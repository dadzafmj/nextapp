...
$y = $y+$dy;
$this->Text($x0+$dx,$y,' N');
$this->Text($x1+$dx,$y,'Medecin Prescripteur');
...
$pdf->Text($xtitre,$ytitre,'STATISTIQUE PAR MEDECIN PRESCRIPTEUR');
if($choix=='periode')
{
	$pdf->SetFont('Arial','',12);
	$pdf->Text($xtitre,$ytitre2,'PERIODE DU '.dateFR($date_debut).' ET '.dateFR($date_fin).'');
	$sql1 = "SELECT DISTINCT `medecin_prescripteur` FROM `patient` WHERE `date_complet` BETWEEN '$date_debut' AND '$date_fin' ORDER BY `medecin_prescripteur`";
}else
{
	$sql1 = "SELECT DISTINCT `medecin_prescripteur` FROM `patient` ORDER BY `medecin_prescripteur`";
}
...
{
$num = $num+1;
$medecin_prescripteur = $res1['medecin_prescripteur'];
$sql2 = "SELECT * FROM `patient` WHERE `medecin_prescripteur` = '$medecin_prescripteur'";
...
while($res2 = mysql_fetch_array($req2))
{
	...
	$pdf->setXY($x1+$dx,$y-$dy);$pdf->Cell($l1+$l2+$l3+$l4+$l5+$l6-2*$dx,$dy+2*$ddy,''.$medecin_prescripteur.'',0,0,'L');
