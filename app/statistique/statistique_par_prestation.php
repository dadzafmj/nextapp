...
$this->Text($x2+$dx,$y,'Consult.');
$this->Text($x3+$dx,$y,'Analyses');
$this->Text($x4+$dx,$y,'Echo');
$this->Text($x5+$dx,$y,'Radio');
$this->Text($x6+$dx,$y,'Cardio');
$this->Text($x7+$dx,$y,'Chirurgie');
$this->Text($x8+$dx,$y,'Autres');
...
$pdf->Text($xtitre,$ytitre,'STATISTIQUE PAR TYPE DE PRESTATION');
if($choix=='periode')
{
...
while($res1 = mysql_fetch_array($req1))
{
	...
	while($res2 = mysql_fetch_array($req2))
	{
		...
		$nbre_total = $nbre_consults + $nbre_analyses + $nbre_echographies + $nbre_radiographies + $nbre_cardiographies + $nbre_chirurgies + $nbre_autres;
	}
		...
		$pdf->setXY($x2+$dx,$y-$dy);$pdf->Cell($l3-2*$dx,$dy+2*$ddy,''.number_format($nbre_consults,0,","," ").'',0,0,'C');
		$pdf->setXY($x3+$dx,$y-$dy);$pdf->Cell($l4-2*$dx,$dy+2*$ddy,''.number_format($nbre_analyses,0,","," ").'',0,0,'C');
		$pdf->setXY($x4+$dx,$y-$dy);$pdf->Cell($l5-2*$dx,$dy+2*$ddy,''.number_format($nbre_echographies,0,","," ").'',0,0,'C');
		$pdf->setXY($x5+$dx,$y-$dy);$pdf->Cell($l6-2*$dx,$dy+2*$ddy,''.number_format($nbre_radiographies,0,","," ").'',0,0,'C');
		$pdf->setXY($x6+$dx,$y-$dy);$pdf->Cell($l7-2*$dx,$dy+2*$ddy,''.number_format($nbre_cardiographies,0,","," ").'',0,0,'C');
		$pdf->setXY($x7+$dx,$y-$dy);$pdf->Cell($l8-2*$dx,$dy+2*$ddy,''.number_format($nbre_chirurgies,0,","," ").'',0,0,'C');
		$pdf->setXY($x8+$dx,$y-$dy);$pdf->Cell($l9-2*$dx,$dy+2*$ddy,''.number_format($nbre_autres,0,","," ").'',0,0,'C');
	...
	$pdf->SetXY($x2+$dx,$y-$dy);$pdf->Cell($l3-2*$dx,$dy+2*$ddy,''.number_format($total_consults,0,","," ").'',0,0,'C');
	$pdf->SetXY($x3+$dx,$y-$dy);$pdf->Cell($l4-2*$dx,$dy+2*$ddy,''.number_format($total_analyses,0,","," ").'',0,0,'C');
	$pdf->SetXY($x4+$dx,$y-$dy);$pdf->Cell($l5-2*$dx,$dy+2*$ddy,''.number_format($total_echographies,0,","," ").'',0,0,'C');
	$pdf->SetXY($x5+$dx,$y-$dy);$pdf->Cell($l6-2*$dx,$dy+2*$ddy,''.number_format($total_radiographies,0,","," ").'',0,0,'C');
	$pdf->SetXY($x6+$dx,$y-$dy);$pdf->Cell($l7-2*$dx,$dy+2*$ddy,''.number_format($total_cardiographies,0,","," ").'',0,0,'C');
	$pdf->SetXY($x7+$dx,$y-$dy);$pdf->Cell($l8-2*$dx,$dy+2*$ddy,''.number_format($total_chirurgies,0,","," ").'',0,0,'C');
	$pdf->SetXY($x8+$dx,$y-$dy);$pdf->Cell($l9-2*$dx,$dy+2*$ddy,''.number_format($total_autres,0,","," ").'',0,0,'C');
